...
def add_category(name):
    conn = sqlite3.connect(DB_FILE); cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO categories (name) VALUES (?)", (name,)); conn.commit()
    ...
    with st.form("edit_main_form"):
        ...
        with col1:
            if st.form_submit_button("💾 Değişiklikleri Kaydet", use_container_width=True):
                update_recipe(st.session_state.recipe_to_edit_id, {'baslik': edit_baslik, 'kategori': edit_kategori, 'malzemeler': edit_malzemeler, 'yapilisi': edit_yapilisi}); st.success(f"Tarif güncellendi!"); st.session_state.recipe_to_edit_id = None; st.rerun()
        with col2:
    ...
    if selected_page == "Tüm Tarifler":
        ...
        if not all_categories_db:
            ...
            for category in all_categories_db:
                ...
                with col1:
                    new_name = st.text_input("Kategori Adı", value=category['name'], key=f"cat_name_{category['id']}")
                with col2:
